...
function Stat({ label, value, accent = 'text-white' }: { label: string; value: string; accent?: string }) {
  return (
    <div className="p-4 rounded-xl bg-panel border border-line space-y-1.5">
      <span className="text-[10px] text-zinc-400 font-mono uppercase font-bold">{label}</span>
...
function Chip({ children }: { children: React.ReactNode }) {
  return (
    <span className="px-2 py-1 rounded-lg bg-card border border-line text-zinc-300">{children}</span>
  );
}
