...
//
// Available=false means the method is not implemented — the UI must not offer
// it as if it worked. That is the honest state for OAuth and for the embedded
// browser today: neither has a worker behind it in this repo.
...
func browserMethod(what string) AuthMethod {
	return AuthMethod{
		Level:   "level-3",
		Name:    "Embedded browser session",
		Summary: "Palius opens " + what + " in a built-in browser window. You log in normally — the same login page you always use. Palius keeps the resulting session cookies; you never install a browser extension, paste a cookie, or hand over your password.",
		// There is no Playwright worker in this repo. Offering this as working
		// would be a lie the moment someone clicked it.
		Available:   false,
		Unavailable: "The embedded browser worker is not running in this deployment. Once it is, this is the universal fallback for any site — including ones with no API.",
		Verifiable:  false,
	}
}
...
func ConnectableCatalog() []ConnectablePlatform {
	social := func(id, name, oauthName, docs string) ConnectablePlatform {
		return ConnectablePlatform{
			...
			Methods: []AuthMethod{oauthMethodFor(id, oauthName, docs), browserMethod(name)},
		}
	}
	...
	return []ConnectablePlatform{
		...
		},
		{
			...
			Methods: []AuthMethod{{
				...
				Summary: "Medium stopped issuing new integration tokens; existing ones still work. Unconfigured or failed publishes fall back to an export.",
		...
		},
		{
			...
			Methods: []AuthMethod{{
				...
				Summary: "Substack has no public write API. Palius prepares a ready-to-paste HTML export instead of failing silently — nothing to connect.",
			}},
		},
		{
...
func handleVerifyConnection(c *gin.Context) {
	id := c.Param("id")
	var platform, enc string
	if err := db.QueryRow(
		`SELECT platform, secrets_enc FROM platform_connections WHERE id = ? AND user_id = ?`,
		id, userId(c)).Scan(&platform, &enc); err != nil {
